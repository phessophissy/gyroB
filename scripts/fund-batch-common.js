...
const EXPECTED_FUNDER_ADDRESS = process.env.FUNDER_ADDRESS || "";
...
export async function runBatchFunding({
  ...
  if (!EXPECTED_FUNDER_ADDRESS) {
    throw new Error("Set FUNDER_ADDRESS to the wallet that will fund batch wallets.");
  }
  ...
  if (funderAccount.address.toLowerCase() !== EXPECTED_FUNDER_ADDRESS.toLowerCase()) {
    throw new Error(`FUNDER_PRIVATE_KEY does not match FUNDER_ADDRESS (${EXPECTED_FUNDER_ADDRESS}).`);
  }
