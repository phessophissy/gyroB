...
const GYROB_CONTRACT_ADDRESS = process.env.GYROB_CONTRACT_ADDRESS || "";
const USDM_ADDRESS = process.env.USDM_ADDRESS || "0x765DE816845861e75A25fCA122bb6898B8B1282a";

function requireContractAddress() {
  if (!GYROB_CONTRACT_ADDRESS) {
    throw new Error("Set GYROB_CONTRACT_ADDRESS to the deployed GyroBoard contract.");
  }
  return GYROB_CONTRACT_ADDRESS;
}
...
export async function runBatchInteractions({
  ...
  if (!room.exists) {
    ...
    throw new Error(
      `Room ${normalizedRoomId.toString()} does not exist on ${contractAddress}. Available rooms: ${knownRoomIds}. Seed rooms before running batch interactions.`,
    );
  }
